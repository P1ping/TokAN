...
from tokan.transformer.decoder_layer import DecoderLayer
...
from tokan.utils.mask import subsequent_mask, make_pad_mask
...
class TransformerDecoder(torch.nn.Module):
            ...
            [
                DecoderLayer(
                    ...
                    COSYVOICE_ATTENTION_CLASSES["selfattn"](
                        attention_heads, attention_dim, self_attention_dropout_rate, key_bias
                    ),
                    (
                        COSYVOICE_ATTENTION_CLASSES["selfattn"](
                            attention_heads, attention_dim, src_attention_dropout_rate, key_bias
                        )
                        if src_attention
                        else None
                    ),
                    PositionwiseFeedForward(attention_dim, linear_units, dropout_rate, activation),
                    dropout_rate,
                    normalize_before,
                )
    ...
    def forward_layers(
        self, x: torch.Tensor, tgt_mask: torch.Tensor, memory: torch.Tensor, memory_mask: torch.Tensor
    ) -> torch.Tensor:
