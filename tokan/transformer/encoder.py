...
from tokan.transformer.convolution import ConvolutionModule
from tokan.transformer.encoder_layer import TransformerEncoderLayer
from tokan.transformer.encoder_layer import ConformerEncoderLayer
from tokan.transformer.positionwise_feed_forward import PositionwiseFeedForward
from tokan.utils.class_utils import (
...
from tokan.utils.mask import add_optional_chunk_mask
...
class BaseEncoder(torch.nn.Module):
    ...
    def forward_chunk(
        ...
        elayers, cache_t1 = att_cache.size(0), att_cache.size(3)
        ...
        for i, layer in enumerate(self.encoders):
            # NOTE(xcsong)(deprecated): Before layer.forward
            #   shape(att_cache[i:i + 1]) is (1, head, cache_t1, d_k * 2),
            #   shape(cnn_cache[i])       is (b=1, hidden-dim, cache_t2)
            # NOTE(qibing):
            #   shape(attn_cache) is (layer, batch, head, cache_t1, d_k * 2),
            #   shape(attn_cache[i]) is (batch, head, cache_t1, d_k * 2)
            xs, _, new_att_cache, new_cnn_cache = layer(
                xs,
                att_mask,
                pos_emb,
                att_cache=att_cache[i] if elayers > 0 else att_cache,
        ...
        # NOTE(xcsong)(deprecated): shape(r_att_cache) is (elayers, head, ?, d_k * 2),
        #   ? may be larger than cache_t1, it depends on required_cache_size
        # NOTE(qibing): shape(r_att_cache) is (elayers, batch, head, ?, d_k * 2)
